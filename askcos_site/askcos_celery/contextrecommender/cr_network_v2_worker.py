...
CORRESPONDING_QUEUE = 'cr_network_v2_worker'
MODEL_CACHE_CAPACITY = 2

def resolve_model_name(model_name):
    '''Resolve the default model name 'fp' and 'graph'.'''
    if model_name in gc.CONTEXT_V2['default-models']:
        model_name = gc.CONTEXT_V2['default-models'][model_name]
    return model_name

def load_new_model(model_cache, model_name):
    '''Load a new model into model cache'''
...
def configure_worker(options={}, **kwargs):
    ...
    print('Context recommendation v2 model will be loaded.')
    print('MODEL_CACHE_CAPACITY={}'.format(MODEL_CACHE_CAPACITY))
    if load_new_model(model_cache, resolve_model_name('fp')) is None:
        raise RuntimeError("Cannot initialize fp model")
    if load_new_model(model_cache, resolve_model_name('graph')) is None:
        raise RuntimeError("Cannot initialize graph model")
...
def get_n_conditions(*args, **kwargs):
    ...
    try:
        ...
        model = load_new_model(model_cache, model_name)
        if model is None:
            return None
